...
from utils.attack_util import logits_margin, attack, get_diff, noise_atk
...
class M3DDPGLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        q_taken = self.critic(inputs[:, :-1], actions[:, :-1].detach()) # detach it to ensure the action only participates in the process of Q backward.
        ...
        target_vals = self.target_critic(inputs[:, 1:], target_actions.detach()) # removed detach since we want to compute epsilon perturbation
        target_vals = target_vals.view(batch_size, -1, 1)
        
        
        target_actions_copy = copy.deepcopy(target_actions)
        target_actions_copy.requires_grad = True
        inputs_copy = copy.deepcopy(inputs)
        inputs_copy.requires_grad = True
        target_value_copy = self.target_critic(inputs_copy[:, 1:], target_actions_copy)
        
        # a = th.randn_like(inputs[:, 1:], requires_grad=True)
        # b = th.randn_like(target_actions, requires_grad=True)
        # c = self.target_critic(a, b)
        # print(th.autograd.grad(c.mean(), a,
        ...
        perturbed_actions = self.perturb_opponent_actions(target_actions_copy, target_value_copy)
        ...
        td_error = q_taken.view(-1, 1) - targets.detach() # (self.n_agents * batch_size)
        ...
        for i in range(self.n_agents):
            ...
            for j in range(self.n_agents):
                if i == j:
                    ...
                    actions_i.append(perturbed_actions[:,j])
    ...
    def perturb_opponent_actions(self, actions, Q):
        # Compute adversarial perturbation based on Minimax principle
        perturbations = []  
        for agent in range(self.n_agents):
            
            Q_i = -Q[:,:,agent]
            action = actions[:, :, agent, agent * self.n_actions:(agent+1) * self.n_actions]
            grad = th.autograd.grad(Q_i.mean(), actions,
                            retain_graph=True, create_graph=True)[0]
            perturb = self.args.adv_eps * grad  # Apply small adversarial perturbation
            perturbed_action = action + perturb[:, :, agent, agent * self.n_actions:(agent+1) * self.n_actions]
            perturbations.append(perturbed_action)
        return th.stack(perturbations, dim=1)
