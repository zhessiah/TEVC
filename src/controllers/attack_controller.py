...
class AttackMAC(BasicMAC):
    ...
    def select_actions(self, ep_batch, t_ep, t_env, bs=slice(None), test_mode=False, attack="rl"):
        ...
        try:
            which_agent_to_attack = self.attack_agent_selector.select_action(attacker_outputs[bs], t_env, test_mode=test_mode)
        except:
            assert 0
        #attacker_action = self.attacker_action_selector.select_action(attacker_outputs[bs], t_env, test_mode=test_mode)
        #(bs, )
        ori_actions = self.action_selector.select_action(agent_outputs[bs], avail_actions[bs],
                                               t_env, test_mode=test_mode)
        worst_actions = self.action_selector.select_byzantine_action(agent_outputs[bs], avail_actions[bs],
                                               t_env, test_mode=test_mode)
    ...
    def _build_forced_inputs(self, batch, t):
        ...
        if self.args.obs_last_action:
            if t == 0:
                inputs.append(th.zeros_like(batch["do_actions_onehot"][:, t]))
            else:
                inputs.append(batch["do_actions_onehot"][:, t - 1])
